...
bool is_kw(const Token& t, token::Keyword k) {
    return std::holds_alternative<token::Keyword>(t.m_value) &&
           std::get<token::Keyword>(t.m_value) == k;
}

bool is_op(const Token& t, token::Operator o) {
    return std::holds_alternative<token::Operator>(t.m_value) &&
           std::get<token::Operator>(t.m_value) == o;
}

bool is_punct(const Token& t, token::Punctuator p) {
    return std::holds_alternative<token::Punctuator>(t.m_value) &&
           std::get<token::Punctuator>(t.m_value) == p;
}

bool is_assign(const Token& t, token::Assignment a) {
    return std::holds_alternative<token::Assignment>(t.m_value) &&
           std::get<token::Assignment>(t.m_value) == a;
}
...
bool is_int(const Token& t, uint64_t val) {
    if (auto* lit = std::get_if<token::Literal>(&t.m_value)) {
        if (auto* i = std::get_if<token::IntLiteral>(lit))
            return i->value == val;
    }
    return false;
}
...
bool is_float(const Token& t, double val) {
    if (auto* lit = std::get_if<token::Literal>(&t.m_value)) {
        if (auto* f = std::get_if<token::FloatLiteral>(lit))
            return std::abs(f->value - val) < 1e-9;
    }
    return false;
}
...
bool is_str(const Token& t, std::string_view val) {
    if (auto* lit = std::get_if<token::Literal>(&t.m_value)) {
        if (auto* s = std::get_if<token::StringLiteral>(lit))
            return s->raw_text == val;
    }
    return false;
}
...
bool is_bool(const Token& t, bool val) {
    if (auto* lit = std::get_if<token::Literal>(&t.m_value)) {
        if (auto* b = std::get_if<token::BoolLiteral>(lit))
            return b->value == val;
    }
    return false;
}
...
std::vector<Token> lex_ok(std::string_view source) {
    ...
    if (!res.has_value()) {
        throw std::runtime_error(
            std::format("Lexing failed unexpectedly: {}", res.error().message));
    }
    return res.value();
}
...
void test_keywords_and_identifiers() {
    ...
    ASSERT(is_ident(tokens[1], "моя_переменная_1"),
           "Expected identifier 'моя_переменная_1'");
...
void test_string_literals() {
    ...
    ASSERT(is_str(tokens[0], "Привет, мир! \\н \\о \\\" \\\\"),
           "String raw text mismatch");
...
void test_operators_and_punctuators() {
    ...
    ASSERT(std::holds_alternative<token::Spread>(tokens[0].m_value),
           "Expected '...'");
    ...
    ASSERT(is_assign(tokens[6], token::Assignment::PlusAssign),
           "Expected '+='");
    ASSERT(is_assign(tokens[7], token::Assignment::MinusAssign),
           "Expected '-='");
    ASSERT(is_assign(tokens[8], token::Assignment::StarAssign),
           "Expected '*='");
    ASSERT(is_assign(tokens[9], token::Assignment::SlashAssign),
           "Expected '/='");
    ASSERT(std::holds_alternative<token::TypeMorpheme>(tokens[10].m_value),
           "Expected '!'");
    ASSERT(std::holds_alternative<token::TypeMorpheme>(tokens[11].m_value),
           "Expected '?'");
...
void test_comments_and_whitespace() {
    ...
    ASSERT(tokens.size() == 2,
           "Expected exactly 2 tokens (integer 42 and EOF)");
...
void test_lexical_errors() {
    ...
    ASSERT(err1.message.find("не был завершен") != std::string_view::npos,
           "Expected unclosed string error");

    auto err2 = lex_err("\"Invalid escape \\q \"");
    ASSERT(err2.message.find("Неверная экранирующая последовательность") !=
               std::string_view::npos,
           "Expected invalid escape error");

    // Number constraints
    auto err3 = lex_err("05");
    ASSERT(err3.message.find("не может начинаться с нуля") !=
               std::string_view::npos,
           "Expected leading zero error on int");

    auto err4 = lex_err("0ш0А");
    ASSERT(err4.message.find("не может начинаться с нуля") !=
               std::string_view::npos,
           "Expected leading zero error on hex");

    auto err5 = lex_err("0д2");
    ASSERT(err5.message.find("должен начинаться с '0д1'") !=
               std::string_view::npos,
           "Expected bad binary prefix error");

    auto err6 = lex_err("3.э5");
    ASSERT(err6.message.find("ожидалась цифра после точки") !=
               std::string_view::npos,
           "Expected malformed float error");
}

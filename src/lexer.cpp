...
static const std::unordered_map<std::string_view, Token::ValueVariant> KeywordMap = {
    // Keywords
...
std::expected<std::vector<Token>, token::Error> Lexer::tokenize(std::string_view source) {
    Lexer lexer(source);
...
Token Lexer::create_token(Token::ValueVariant value, uint32_t start_pos) const noexcept {
...
Token Lexer::error_token(std::string_view message, uint32_t start_pos) const noexcept {
            ...
            return create_token(
                token::Literal{
                    token::StringLiteral{
                        m_source.substr(
                            start_pos + 1,
                            (m_pos - 1) - (start_pos + 1))}}, start_pos);
        }
        if (c == '\n') {
            return error_token("Перенос строки внутри строкового литерала недопустим.\n", start_pos);
        }
        if (c == '\\') {
            advance_char();
            if (m_pos >= m_source.length()) return error_token("Экранирующая последовательность не была завершена", start_pos);
            ...
            if (char d = current(); d == '"' || d == '\\' || d == '0') {
                ...
                return error_token("Неверная экранирующая последовательность", start_pos);
            }
        } else {
            advance_char();
        }
    }
    return error_token("Строковый литерал не был завершен двойной кавычкой '\"'", start_pos);
}
...
Token Lexer::extract_number(uint32_t start_pos) noexcept {
    if (current() == '0') {
        ...
        if (std::isdigit(static_cast<unsigned char>(current()))) {
            return error_token("Ненулевой численный литерал не может начинаться с нуля", start_pos);
        }
    ...
    uint64_t int_part = 0;
    while (m_pos < m_source.length() && std::isdigit(static_cast<unsigned char>(current()))) {
...
Token Lexer::extract_invalid_float(uint32_t start_pos) noexcept {
    ...
    // Consume the malformed exponent part if present for a wider, clearer error span
    ...
    return error_token("Неверный формат числа с плавающей точкой: ожидалась цифра после точки", start_pos);
}
...
Token Lexer::extract_hex(uint32_t start_pos) noexcept {
    ...
    if (c >= '1' && c <= '9') {
        ...
        advance_char();
        first_valid = true;
    } else if (match_sequence("\xD0\x90")) { val = 10; m_pos += 2; first_valid = true; } // А
    else if (match_sequence("\xD0\x91")) { val = 11; m_pos += 2; first_valid = true; } // Б
    else if (match_sequence("\xD0\x92")) { val = 12; m_pos += 2; first_valid = true; } // В
    else if (match_sequence("\xD0\x93")) { val = 13; m_pos += 2; first_valid = true; } // Г
    else if (match_sequence("\xD0\x94")) { val = 14; m_pos += 2; first_valid = true; } // Д
    else if (match_sequence("\xD0\x95")) { val = 15; m_pos += 2; first_valid = true; } // Е

    if (!first_valid) {
        if (c == '0') return error_token("Шестнадцатеричный литерал не может начинаться с нуля\n(нуль допустим только без приставки: '0')", start_pos);
        return error_token("Ожидалась ненулевая шестнадцатеричная цифра после '0ш'", start_pos);
    }
    ...
    while (m_pos < m_source.length()) {
        ...
        if (std::isdigit(static_cast<unsigned char>(c))) {
            val = val * 16 + (c - '0');
            advance_char();
        } else if (match_sequence("\xD0\x90")) { val = val * 16 + 10; m_pos += 2; }
        else if (match_sequence("\xD0\x91")) { val = val * 16 + 11; m_pos += 2; }
        else if (match_sequence("\xD0\x92")) { val = val * 16 + 12; m_pos += 2; }
        else if (match_sequence("\xD0\x93")) { val = val * 16 + 13; m_pos += 2; }
        else if (match_sequence("\xD0\x94")) { val = val * 16 + 14; m_pos += 2; }
        else if (match_sequence("\xD0\x95")) { val = val * 16 + 15; m_pos += 2; }
        else break;
    }
...
Token Lexer::extract_octal(uint32_t start_pos) noexcept {
    ...
    if (c >= '1' && c <= '7') {
        ...
        if (c == '0') return error_token("Восьмеричный литерал не может начинаться с нуля\n(нуль допустим только без приставки: '0')", start_pos);
        return error_token("Ожидалась восьмеричная цифра после '0в'", start_pos);
    }
...
Token Lexer::extract_binary(uint32_t start_pos) noexcept {
    ...
    while (m_pos < m_source.length() && (current() == '0' || current() == '1')) {
        val = (val << 1) | (current() - '0');
        advance_char();
    }
    return create_token(token::Literal{token::IntLiteral{val}}, start_pos);
...
Token Lexer::extract_float_remainder(uint32_t start_pos, uint64_t int_part) noexcept {
    advance_char(); // Consume '.'
    ...
    if (match_sequence("\xD1\x8D")) {
        ...
        if (current() == '0') {
            return error_token("Экспонента числа с плавающей точкой не может начинаться с нуля", start_pos);
        }
        if (!std::isdigit(static_cast<unsigned char>(current()))) {
            return error_token("Ожидались цифры после экспоненты числа с плавающей точкой 'э'", start_pos);
        }
...
Token Lexer::extract_operator_or_punct(uint32_t start_pos) noexcept {
    ...
    switch (c) {
        case '(': return create_token(token::Punctuator::LParen, start_pos);
        case ')': return create_token(token::Punctuator::RParen, start_pos);
        case '{': return create_token(token::Punctuator::LBrace, start_pos);
        case '}': return create_token(token::Punctuator::RBrace, start_pos);
        case '[': return create_token(token::Punctuator::LBracket, start_pos);
        case ']': return create_token(token::Punctuator::RBracket, start_pos);
        case ',': return create_token(token::Punctuator::Comma, start_pos);
        case ':': return create_token(token::Punctuator::Colon, start_pos);
        case ';': return create_token(token::Punctuator::Semicolon, start_pos);
        case '\\': return create_token(token::Punctuator::Backslash, start_pos);
        case '?': return create_token(token::TypeMorpheme::Optional, start_pos);
        case '~': return create_token(token::Operator::BitNot, start_pos);
        case '%': return create_token(token::Operator::Mod, start_pos);
        case '^': return create_token(token::Operator::BitXor, start_pos);
        ...
        case '&': return create_token(token::Operator::BitAnd, start_pos);
        case '|': return create_token(token::Operator::BitOr, start_pos);

        default: return error_token("Недопустимый символ", start_pos);
    }
}
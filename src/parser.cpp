...
// 1. Check if token belongs to a broad category (e.g.,
// token.is<token::Literal>())
...
template <typename T> const T& expect() const {
    if (!is<T>()) {
        throw std::runtime_error(
            std::format("Unexpected token at {}:{}", m_loc.line, m_loc.column));
        // In the final compiler, this will tie into the Diagnostic Builder
        // instead of throwing
    }
    return get<T>();
}
...
void expect(T exact_val) const {
    if (!is(exact_val)) {
        throw std::runtime_error(std::format(
            "Unexpected exact token at {}:{}", m_loc.line, m_loc.column));
    }
}

...
// NOTE: restrict syntax grammar to make parser stricter in order to simplify semantic analyzer's work
...
public:
    ...
    [[nodiscard]] const SourceLocation& location() const noexcept { return m_loc; }
...
public:
    ...
    // For `стат_ряд`. Variant captures literal size OR compile-time generic identifier.
...
public:
    ...
    void accept(Visitor& v) const override { v.visit(*this); }
    std::vector<std::string_view> qualified_path; // Identifiers for namespace resolution
};
...
public:
    BinaryExpr(SourceLocation loc, token::Operator op, ExprPtr left, ExprPtr right)
...
public:
    CallExpr(SourceLocation loc, IdentExprPtr target, std::vector<Argument> args)
        : Expr(loc), target_path(std::move(target)), arguments(std::move(args)) {}
...
public:
    IndexSliceExpr(SourceLocation loc, IdentExprPtr target, ExprPtr index, ExprPtr slice_end = nullptr)
        : Expr(loc), target(std::move(target)), index(std::move(index)), slice_end(std::move(slice_end)) {}
...
public:
    MemberAccessExpr(SourceLocation loc, IdentExprPtr target, std::string_view member)
...
public:
    StructInitExpr(SourceLocation loc, IdentExprPtr target, std::vector<ExprPtr> fields)
        : Expr(loc), target_path(std::move(target)), fields(std::move(fields)) {}
    
    void accept(Visitor& v) const override { v.visit(*this); }
    IdentExprPtr target_path;
    std::vector<ExprPtr> fields; // nullptr represents "ничто" explicit passing
};
...
public:
    CastExpr(SourceLocation loc, TypePtr target_type, ExprPtr operand)
        : Expr(loc), target_type(std::move(target_type)), operand(std::move(operand)) {}
...
class IfStmt : public Stmt { // maybe I should restrict syntax and parser in order to simplify semantic analyzer's work
public:
    IfStmt(SourceLocation loc, ExprPtr cond, BlockStmtPtr then_b, StmtPtr else_b = nullptr)
        : Stmt(loc), condition(std::move(cond)), then_block(std::move(then_b)), else_block(std::move(else_b)) {}
    
    void accept(Visitor& v) const override { v.visit(*this); }
    ExprPtr condition;
    BlockStmtPtr then_block;
    StmtPtr else_block; // Can be BlockStmt or another IfStmt (for "иначе если")
};
...
public:
    ReturnStmt(SourceLocation loc, ExprPtr val = nullptr)
        : Stmt(loc), value(std::move(val)) {} // nullptr denotes `вернуть ничто;`
...
public:                                 // v~~~ postponed to semantic analyzer
    AssignmentStmt(SourceLocation loc, ExprPtr lval, token::Assignment op, std::variant<ExprPtr, std::unique_ptr<AssignmentStmt>> rval)
        : Stmt(loc), lvalue(std::move(lval)), op(op), rvalue(std::move(rval)) {}
    
    void accept(Visitor& v) const override { v.visit(*this); }
    ExprPtr lvalue; // Must syntactically map to LValue constraints
    token::Assignment op;
    std::variant<ExprPtr, std::unique_ptr<AssignmentStmt>> rvalue; // Support `a = b = c;`
};
...
public:
    VarDecl(SourceLocation loc, bool is_const, std::string_view name, TypePtr type, ExprPtr init)
        : Decl(loc), is_const(is_const), name(name), type(std::move(type)), initializer(std::move(init)) {}
...
class StructDecl : public Decl { // maybe <qualified_id> should be allowed
public:
    StructDecl(SourceLocation loc, bool is_static, std::string_view name, std::vector<StructField> fields)
        : Decl(loc), is_static(is_static), name(name), fields(std::move(fields)) {}
...
public:
    FuncDecl(SourceLocation loc, std::string_view name, std::vector<Parameter> params, TypePtr ret_type, bool must_use, BlockStmtPtr body)
        : Decl(loc), name(name), params(std::move(params)), return_type(std::move(ret_type)), must_use(must_use), body(std::move(body)) {}
...
public:
    IncludeStmt(SourceLocation loc, std::string_view file_path, std::vector<std::string_view> imports)
        : ModuleStmt(loc), file_path(file_path), specific_imports(std::move(imports)) {}

    void accept(Visitor& v) const override { v.visit(*this); }
    std::string_view file_path;
    std::vector<std::string_view> specific_imports; // Empty means `включить "файл.ru"` completely
};
...
public:
    UsingStmt(SourceLocation loc, std::string_view alias, TypePtr target_type)
        : ModuleStmt(loc), alias_name(alias), target_type(std::move(target_type)), is_namespace_wildcard(false) {}

    UsingStmt(SourceLocation loc, IdentExprPtr ns_path)
        : ModuleStmt(loc), target_namespace(std::move(ns_path)), is_namespace_wildcard(true) {}

    void accept(Visitor& v) const override { v.visit(*this); }
    std::string_view alias_name;
    TypePtr target_type; // Set if alias to Type
    
    IdentExprPtr target_namespace; // Set if alias to namespace `употреб Область\*`
    bool is_namespace_wildcard;
};

class NamespaceStmt : public ModuleStmt { // restrict member type respectively to grammar
public:
    NamespaceStmt(SourceLocation loc, std::string_view name, std::vector<StmtPtr> members)

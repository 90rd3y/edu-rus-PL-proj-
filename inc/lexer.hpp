...
class Lexer {
  public:
    ...
    [[nodiscard]] static std::expected<std::vector<Token>, token::Error>
  ...
  private:
    ...
    [[nodiscard]] std::expected<std::vector<Token>, token::Error>
    tokenize() noexcept;
    ...
    Token create_token(Token::ValueVariant value,
    ...
    Token extract_float_remainder(uint32_t start_pos,
